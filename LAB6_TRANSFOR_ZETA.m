...
% Last Modified by GUIDE v2.5 02-Dec-2016 16:06:57
...
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
global A xn  %Declaramos variables globales
syms Xz n z %Declaramos variables simbolicas
...
P = strcat('$$', 'X(z) = ', char(latex(Xz)),'$$'); %Construimos la cadena que imprimira F(w)
...
cla %Limpiamos el espacio para  el grafico
Xzm=abs(Xz); %Generamos el espectro de magnitud
z=A(1):1/100:A(2);  %Generamos el vector de frecuencia angular
if Xzm~=Xz %Verificamos si la funcion es constante
 plot(z,eval(Xz),'Color', 'red', 'Linewidth', 1.3);hold on %Graficamos el espectro de magnitud
else
 plot(z,eval(Xz*power(z,0)),'Color', 'red', 'Linewidth', 1.3);hold on %Graficamos el espectro de magnitud
end
...
xlabel('\bfFRECUENCIA'); %Leyenda del eje x
ylabel('\bfAMPLITUD'); %Leyenda del eje y
title('\bfX(z)') %Titulo del grafico
...
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
global A xn  %Definimos variables globales
syms n %Activamos la grafica para el espectro de Fase
...
xn=eval(get(handles.ECUACION, 'String')); %Se obtiene la ecuacion de la señal del primer tramo
xnp=abs(xn);
n=A(1):1/5:A(2); %Generamos el vector del tiempo para la señal completa
if xnp~=xn %Verificamos si la funcion es constante
 stem(n,eval(xn),'-o','Color', 'blue', 'Linewidth', 1.3);hold on %Graficamos el espectro de magnitud
else
 xni=subs(xn,n);
 stem(n,eval(xni),'-o','Color', 'blue', 'Linewidth', 1.3);hold on %Graficamos el espectro de magnitud
end
xlim auto%limite del eje x de la grafica
ylim auto%limite del eje y de la grafica
grid on  %Establecemos cuadriculas al grafico
